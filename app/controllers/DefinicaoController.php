<?php
class DefinicaoController extends \HXPHP\System\Controller
{
	...
	public function registrarAction()
	{
		...
		if($institution->nivel >= 4)
		{
			if (!empty($post)){
			$evt = DefinitionHearings::registrar($user_id, $post);
			if($evt->status === true)
			{
				$institution->nivel = 5;
				...
				for ($i = 0; $i < $num_file; $i++) 
				{
					...
					{		
						...
						if(in_array($extension, $allowedExts))
						{
							...
							$reportsUpload = DefinitionHearings::enviar($array_file[$i], $evt->reports->id);
				...
				array_push($callbackMsg->msgm, 'Relatório de audiência de definição criado.');
				$this->load('Helpers\Alert', array(
						'success',
						'Sucesso',
						$callbackMsg->msgm
	...
	public function atualizarAction()
	{
		...
		$evt = DefinitionHearings::find_by_id($id);
		...
		if (!empty($post)){
			...
			for ($i = 0; $i < $num_file; $i++) 
			{
				...
				{		
					...
					{
						//deve passar o id do relatorio financieiro
						$reportsUpload = DefinitionHearings::enviar($array_file[$i], $id);
			...
			array_push($callbackMsg->msgm, 'Relatório de audiência de definição atualizado.');
			$this->load('Helpers\Alert', array(
					'success',
					'Sucesso',
					$callbackMsg->msgm
	...
	{	
		...
		$evt = DefinitionHearings::find_all_by_cnpj($cnpj);
		...
		if(empty($evt)){
			$this->load('Helpers\Alert', array(
					'danger',
					'Nenhum relatório de audência encontrado.'
				));
