<?php
class ApresentacaoController extends \HXPHP\System\Controller
{
	...
	public function registrarAction()
	{
		...
		if($institution->nivel >= 5)
		{	
			if (!empty($post))
			{
				$evt = ApresentationHearings::registrar($user_id, $post);
					...
					for ($i = 0; $i < $num_file; $i++) 
					{
						...
						{	
							...
							if(in_array($extension, $allowedExts))
							{
								...
								$reportsUpload = ApresentationHearings::enviar($array_file[$i], $evt->reports->id);
	...
	public function atualizarAction()
	{
		...
		$evt = ApresentationHearings::find_by_id($id);
		...
		if (!empty($post)){
			...
			for ($i = 0; $i < $num_file; $i++) 
			{
				...
				{		
					...
					{
						//deve passar o id do relatorio financieiro
						$reportsUpload = ApresentationHearings::enviar($array_file[$i], $id);
	...
	{	
		...
		$evt = ApresentationHearings::find_all_by_cnpj($cnpj);
		...
		$this->view->setTitle('MIS - Administrativo')
			...
			->setVars([
				'audiencia' => $evt,
				//'evts' => ApresentationHearings::all()
			]);}
	}
}