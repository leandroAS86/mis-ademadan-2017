...
class Closures extends \HXPHP\System\Model
{
	...
	public static function registrar($user_id, array $post)
	{
		...
		//var_dump($post);
		$exists_nome = self::find_by_projeto($post['projeto']);
		
		if (!is_null($exists_nome)) {
			array_push($callbackObj->errors, 'Relatório de enceramento para projeto com mesmo título já foi criado. Se precisar atualize o relatório.');
			return $callbackObj;
		}
		...
		if($cadastrar->is_valid()){
			...
			array_push($callbackObj->msg, 'Relatório de evento de encerramento foi criado.');
			return $callbackObj;
		}
