...
class FinancialReports extends \HXPHP\System\Model
{
	...
	public static function registrar($user_id, array $post)
	{
		...
		$cadastrar = self::create(array(
			...
			'acao'=>$post['acao'],
			'desembolso_data'=>$post['desembolso_data'],
			'desembolso_valor'=>$post['desembolso_valor'],			
		));

		if($cadastrar->is_valid()){
			$callbackObj->reports=$cadastrar;
			$callbackObj->status=true;
			array_push($callbackObj->msg, 'Relatório financiro foi criado.');
			return $callbackObj;
		}
